...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        #Initialize variables similar to QueensConstraint's init
        
        scope = [qi, qj]
        #Keep track of assignments that are valid(satisfying)
        validAssignments = []
        
        #Compare the domains of both variables
        for iDomain in qi.domain():
           for jDomain in qj.domain():
                #Check to see if they are not in same column and not diagonal
                if iDomain != jDomain and abs(iDomain - jDomain) != abs(i - j):
                    #Found valid pair(tuple)
                    validAssignments.append([iDomain, jDomain])
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        #Following the aproach in AllDiffConstraint.check
        valueAssignments = []
        #Go through each var in the scope
        for possibleVariable in self.scope():
            #Check if the variable's value is the required one
            if possibleVariable.getValue() == self._required:
                #add to our list
                valueAssignments.append(possibleVariable)
        #check if the number of variables are in the correct range
        #which means the amount of assignments does not exceed the upper bound 
        #and has at least lower bound quantity 
        return self._lb <= len(valueAssignments) <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        '''
        #Following the approach in AllDiffConstraint.hasSupport
        if var not in self.scope():
            #var = val has support on any constraint it does not participate in
            return True
        
        def valsNotEqual(l):
            #Following AllDiffConstraint.valsNotEqual
            #Similar to check, except uses different input
            #track the values Assigned
            valueAssignments = []
            
            #Get all the values from the list
            vals = [val for (var, val) in l]
            for possibleVariable in vals:
                #Check if the variable's value is the required one
                if possibleVariable == self._required:
                    #add to our list
                    valueAssignments.append(possibleVariable)
            #check if the number of variables are in the correct range
            #which means the amount of assignments does not exceed the upper bound 
        ...
        #Following Taken from AllDiffConstraint
        varsToAssign = self.scope()
        varsToAssign.remove(var)
