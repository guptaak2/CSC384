...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            #get all diff constraint for row using provided class
            rowConstraint = AllDiffConstraint('AllDiffRow', row)
			#add to constraint list
            constraint_list.append(rowConstraint)
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            #get all diff constraint for column
            columnConstraint = AllDiffConstraint('AllDiffColumn', scope)
            #add to constraint list
            constraint_list.append(columnConstraint)
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                #Get all diff for the subsquare
                subSquare = AllDiffConstraint('AllDiffSquare', scope)
                #append to the constraints list
                constraint_list.append(subSquare)
