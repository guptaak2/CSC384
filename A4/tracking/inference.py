...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        for p in self.legalPositions:
            #get the true distance using manhattanDistance
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        #loop over each legal position
        for old_p in self.legalPositions:
            #calculate the new distance
            new_dist = self.getPositionDistribution(self.setGhostPosition(gameState, old_p))
            for new_p, prob in new_dist.items():
                allPossible[new_p] += self.beliefs[old_p]*prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #Get the current beliefs
        self.beliefs = util.Counter()
        for p in self.legalPositions:
            #update the belief for each of the positions to 1 (initial)
            self.beliefs[p] = 1.0
        self.beliefs.normalize()
        "*** END YOUR CODE HERE ***"
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter()
        for p in self.legalPositions:
            #calculate the distance for position
            trueDistance = util.manhattanDistance(p, pacmanPosition)
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # same as elapseTime function for ExactInference (above)
        #get all positions
        allPossible = util.Counter()
        for old_p in self.legalPositions:
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #per requirement above, create a list
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            ...
            for g in range(self.numGhosts):
                manhattan_dist = util.manhattanDistance(ghost_pos[g], pacmanPosition)
                if noisyDistances[g] is None:
                    #Ghost in jail
                    p = self.getParticleWithGhostInJail(p, g)
                else:
                    #Ghost not in jail
                    weight = weight * emissionModels[g][manhattan_dist]
            weights[p] = weights[p] + weight
        weights.normalize()

        if weights.totalCount() != 0:
            for p in range(self.numParticles):
                particles.append(util.sample(weights))
        else:
            #Per requirement, re initialize a new set of particles
            self.initializeParticles()
            particles = self.particles
            for g in range(self.numGhosts):
                if noisyDistances[g] is None:
                    for i in range(self.numParticles):
                        particles[i] = self.getParticleWithGhostInJail(self.particles[i], g)
    ...
    def getBeliefDistribution(self):
        ...
        dist = util.Counter()
        for p in self.particles: 
            #update the distance for each particle to 1
            dist[p] += 1
